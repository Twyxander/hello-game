...
int main()
{
    ...
    ALLEGRO_DISPLAY* disp = al_create_display(2700, 1700);
   ...
   if (!font){
    ...
    A=al_load_bitmap("A.png");

    if (!A){
      fprintf(stderr, "Could not load 'A.png'.\n");
      return -1;
   }

    B=al_load_bitmap("B.png");

    if (!B){
      fprintf(stderr, "Could not load 'B.png'.\n");
      return -1;
    }
    ...
    while(1)
    {
        ...
        {            
            ...
            al_draw_bitmap(A, posuv_A_x, posuv_A_y, 0);
            al_draw_bitmap(B, posuv_B_x, posuv_B_y, 0);

            //al_draw_rectangle(posuv_A_x, posuv_A_y, posuv_A_x + 10, posuv_A_y + 10, al_map_rgb(255,  0, 0),30.0);
            //al_draw_rectangle(posuv_B_x, posuv_B_y, posuv_B_x + 10, posuv_B_y + 10, al_map_rgb(0, 255, 0),30.0);
            ...
            */
            ...
            al_draw_text(font, al_map_rgb(100, 0, 0), 255, 155, 0, "Hello John this is G world and big game!");
            al_flip_display();

            posuv_A_x = posuv_A_x + zmena_A_x;
            posuv_A_y = posuv_A_y + zmena_A_y;
            
            posuv_B_x = posuv_B_x + zmena_B_x;
            posuv_B_y = posuv_B_y + zmena_B_y;
         
            //if (posuv_x>100 && zmena_A_x==2) zmena_A_y = -2;
            
            /*
            if (posuv_x>200 && zmena_A_x==2)
            {
                zmena_A_x =-2;
                zmena_A_y = 0;
            } 
